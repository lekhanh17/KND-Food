  ...
  return (
    <div className="fixed top-20 left-4 right-4 z-40 space-y-2">
      {notifications.map(notification => (
        <div key={notification.id} className="bg-orange-500 text-white p-4 rounded-lg shadow-lg">
          <h4 className="font-semibold">{notification.title}</h4>
          <p className="text-sm mt-1">{notification.message}</p>
          <span className="text-xs opacity-75 mt-1 block">{notification.time}</span>
        </div>
      ))}
    </div>
  );
