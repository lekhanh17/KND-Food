    ...
    <div className="relative w-full max-w-md" ref={searchRef}>
      ...
      <div className="relative">
        ...
        <input
          ...
          onFocus={() => { if(searchTerm) setShowDropdown(true) }} // Click lại vào ô thì mở lại dropdown
      ...
      {showDropdown && (results.recipes.length > 0 || results.users.length > 0) && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden z-50">
          
          {/* Gợi ý Công thức */}
          {results.recipes.length > 0 && (
            <div className="p-2">
              <h3 className="text-xs font-bold text-gray-400 uppercase px-3 pb-1">Công thức</h3>
              {results.recipes.map((recipe) => (
                ...
                >
                  <img src={recipe.ImageURL || 'link-anh-mac-dinh.jpg'} alt="" className="w-10 h-10 rounded-md object-cover" />
                  <span className="text-sm font-medium text-gray-700 truncate">{recipe.Title}</span>
                </Link>
              ))}
            </div>
          )}
          ...
          {/* Đường kẻ chia cách */}
          {results.recipes.length > 0 && results.users.length > 0 && <hr className="border-gray-100" />}

          {/* Gợi ý Người dùng */}
          {results.users.length > 0 && (
            <div className="p-2">
              <h3 className="text-xs font-bold text-gray-400 uppercase px-3 pb-1">Người dùng</h3>
              {results.users.map((user) => (
                <Link 
                  ...
                  className="flex items-center gap-3 px-3 py-2 hover:bg-orange-50 rounded-lg transition-colors"
                >
                  <img src={user.Avatar || 'link-avatar-mac-dinh.png'} alt="" className="w-8 h-8 rounded-full object-cover border border-gray-200" />
      ...
      {showDropdown && !isSearching && results.recipes.length === 0 && results.users.length === 0 && searchTerm.trim() !== "" && (
         <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-xl shadow-lg border border-gray-100 p-4 text-center z-50">
            <p className="text-sm text-gray-500">Không tìm thấy kết quả nào cho "{searchTerm}"</p>
         </div>
      )}
    </div>
  );
