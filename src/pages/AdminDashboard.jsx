...
import { faTrash, faUserShield, faUsers, faArrowLeft, faUtensils, faCheck, faTimes } from '@fortawesome/free-solid-svg-icons';
...
export default function AdminDashboard() {
  ...
  const handleApproveRecipe = async (id) => {
    ...
    try {
      ...
      toast.error("Lỗi kết nối Server!", toastConfig);
    }
  };

  // --- API TỪ CHỐI BÀI (MỚI) ---
  const handleRejectRecipe = async (id) => {
    const isConfirm = window.confirm("❌ Bạn có chắc chắn muốn từ chối và XÓA bài đăng này không?");
    if(!isConfirm) return;

    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/admin/reject-recipe/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });

      if (response.ok) {
        toast.success("Đã xóa bài đăng vi phạm!");
        setPendingRecipes(pendingRecipes.filter(r => r.RecipeID !== id));
      }
    } catch (error) {
      ...
      <div className="container mx-auto px-4 lg:px-10">
        ...
        <div className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden mb-8">
          <div className="bg-gradient-to-r from-red-600 to-orange-500 px-10 py-12 flex justify-between items-center text-white">
            <div>
              <h1 className="text-3xl font-black uppercase tracking-tighter flex items-center gap-3">
                <FontAwesomeIcon icon={faUserShield} /> {currentUser?.Role === 'Admin' ? 'Quản Trị Hệ Thống' : 'Khu Vực Nhân Viên'}
              </h1>
              <p className="text-red-100 mt-2 font-medium opacity-90">Xin chào, {currentUser?.FullName}</p>
            </div>
          ...
          <div className="flex border-b border-gray-100">
            ...
            >
              <FontAwesomeIcon icon={faUsers} className="mr-2" /> Quản lý thành viên ({users.length})
            </button>
            <button 
            ...
            >
              <FontAwesomeIcon icon={faUtensils} className="mr-2" /> Duyệt công thức ({pendingRecipes.length})
              {pendingRecipes.length > 0 && <span className="ml-2 w-2 h-2 bg-red-500 inline-block rounded-full animate-ping"></span>}
            </button>
          </div>
        </div>
        ...
        <div className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 p-8 min-h-[400px]">
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="border-b border-gray-100">
                        <th className="py-4 px-4 text-xs font-black text-gray-400 uppercase">Thành viên</th>
                        <th className="py-4 px-4 text-xs font-black text-gray-400 uppercase">Email</th>
                        <th className="py-4 px-4 text-xs font-black text-gray-400 uppercase">Vai trò</th>
                        <th className="py-4 px-4 text-xs font-black text-gray-400 uppercase text-center">Thao tác</th>
                      </tr>
                    </thead>
                    <tbody>
                      {users.map((u) => (
                        <tr key={u.UserID} className="border-b border-gray-50 hover:bg-gray-50">
                          <td className="py-5 px-4 font-bold text-gray-800">{u.FullName}</td>
                          ...
                          <td className="py-5 px-4 text-center">
                            {currentUser?.Role === 'Admin' && ( // CHỈ ADMIN THẤY NÚT XÓA
                              <button onClick={() => handleDeleteUser(u.UserID)} className="text-red-400 hover:text-red-600"><FontAwesomeIcon icon={faTrash} /></button>
                            )}
                          </td>
                        </tr>
                      ))}
          ...
          {activeTab === 'recipes' && (
             <>
             <h2 className="text-2xl font-black text-gray-800 uppercase tracking-tighter mb-8 px-2">Công thức chờ duyệt</h2>
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
               {pendingRecipes.length === 0 ? (
                 <div className="col-span-full py-20 text-center text-gray-500 font-bold">Hura! Không có món ăn nào cần duyệt.</div>
               ) : (
                 pendingRecipes.map(recipe => (
                   <div key={recipe.RecipeID} className="border border-gray-100 rounded-3xl p-4 shadow-sm hover:shadow-md transition bg-white">
                     <img src={recipe.ImageURL || 'https://via.placeholder.com/300'} alt={recipe.Title} className="w-full h-40 object-cover rounded-2xl mb-4" />
                     <h3 className="font-black text-lg text-gray-800 line-clamp-1">{recipe.Title}</h3>
                     <p className="text-sm text-gray-500 mt-1">Tác giả: <span className="font-bold">{recipe.FullName}</span></p>
                     
                     <div className="flex gap-2 mt-4">
                       <button onClick={() => handleApproveRecipe(recipe.RecipeID)} className="flex-1 bg-green-500 hover:bg-green-600 text-white py-2 rounded-xl font-bold text-sm transition">
                         <FontAwesomeIcon icon={faCheck} className="mr-1"/> Duyệt
                       </button>
                       <button onClick={() => handleRejectRecipe(recipe.RecipeID)} className="flex-1 bg-red-100 hover:bg-red-200 text-red-600 py-2 rounded-xl font-bold text-sm transition">
